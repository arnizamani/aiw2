...
extractConcepts time (Item d lhs _   r _) | r < 1
    =    [C (0,0,1,time,time,d,makeR c) | c <- getConsts lhs]
      ++ [C (1,0,1,time,time,d,makeR u) | u <- getUnary lhs]
      ++ [C (2,0,1,time,time,d,makeR b) | b <- getBinary lhs]
extractConcepts time (Item d lhs rhs _ _)
    =    [C (0,0,1,time,time,d,makeR c) | c <- getConsts lhs]
      ++ [C (1,0,1,time,time,d,makeR u) | u <- getUnary lhs]
      ++ [C (2,0,1,time,time,d,makeR b) | b <- getBinary lhs]
          ...
          then return . Left . head $ ["Error parsing axiom line " ++ show i | (i,Nothing) <- axiomLines]
           ...
           then return . Left . head $ ["Error parsing concept line " ++ show i | (i,Nothing) <- conceptLines]
            ...
            then return . Left . head $ ["Error parsing edge line " ++ show i | (i,Nothing) <- edgeLines]
              ...
              let patterns = [((d,e1,e2),(b,set)) | (_,Just (Edge (d,e1,e2,b,set))) <- edgeLines]
              (width,wwarn) <- do
              ...
              let domains = nub' [d | ((d,_,_),_) <- patterns]
              let patt     = Set.fromList
                                  $ (concat [[(d,x),(d,y)] | ((d,x,y),_) <- patterns])
                                    ++ [(d,Var [v]) | v <- "xyz", d <- domains]
              let rew      = Set.fromList [Rew (arity,s) | C (arity,rew,_,_,_,_,(Root s)) <- Set.toList concepts, rew > 0]
...
readDomain = do
    skipSpaces
    domain <- munch (\x -> not (isSpace x) && x /='⊨')
    skipSpaces
    char '⊨'
    return domain
