...
langItem s = do
        ...
        then do
            ...
            then do
                let item = Item "English" correctLhs ok (size correctLhs) Nothing
                return item
            else do    -- wrong item with positive viability
                let item = Item "English" wrongLhs (Root "*OK") (size wrongLhs) Nothing
                return item
        else do
            if correct -- correct item with negative viability
            then do
                let item = Item "English" correctLhs (Root "*OK") (0 - (size correctLhs)) Nothing
                return item
            else do
                let item = Item "English" wrongLhs ok (0 - size wrongLhs) Nothing
                return item
