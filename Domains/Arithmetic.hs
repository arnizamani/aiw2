...
arithItem s = do
        ...
        then arithItem s
         ...
         if pos < 1
         then do
             return $ Item "Arith" lhs rhsN viabN Nothing
         else do
             if lhs == rhsP
                 then arithItem s
                 else return $ Item "Arith" lhs rhsP viabP Nothing
...
additionItem s | s <= 10 = do
        ...
        return $ Item "Arith" (Binary (Oper "#") (Root "0") right) right 1 Nothing
additionItem s | s <= 110 = do
        ...
        then additionItem s
         ...
         return $ Item "Arith" lhs rhs 1 Nothing
...
addItems = do
    let numbers = [0..20]
    let digits = [0..9]
    let items0 = [Item "Arith" (makeB "+" (makeR $ show d1) (makeR $ show d2)) (makeExp (d1+d2)) 1 Nothing | d1 <- digits, d2 <- digits]
    let items1 = [Item "Arith" (makeB "#" (makeR "0") (makeExp n)) (makeExp n) 1 Nothing | n <- numbers]
    let items2 = [Item "Arith" (makeB "+" (makeR "0") (makeExp n)) (makeExp n) 1 Nothing | n <- numbers]
    
    let items3 = [Item "Arith" (makeB "+" (makeB "#" (makeR $ show d1) (makeR "0")) (makeR $ show d2)) (makeExp (d1*10+d2)) 1 Nothing
                        | d1 <- take 5 digits, d2 <- take 5 digits]
    let items4 = concat $ -- d1 # (d2 # d3),  (d1 + d2) # d3
                  [[Item "Arith" (makeB "#" (makeR $ show d1) (makeB "#" (makeR $ show d2) (makeR $ show d3))) (makeExp ((d1+d2)*10+d3)) 1 Nothing,
                    Item "Arith" (makeB "#" (makeB "+" (makeR $ show d1) (makeR $ show d2)) (makeR $ show d3)) (makeExp ((d1+d2)*10+d3)) 1 Nothing,
                    Item "Arith" (makeB "#" (makeB "+" (makeR $ show d2) (makeR $ show d1)) (makeR $ show d3)) (makeExp ((d1+d2)*10+d3)) 1 Nothing
                    ]
                        | d1 <- take 5 digits, d2 <- take 5 digits, d3 <- take 5 digits]
    let items5 = [Item "Arith" (makeB "+" (makeExp n1) (makeExp n2)) (makeExp (n1+n2)) 1 Nothing | n1 <- numbers, n2 <- numbers]
