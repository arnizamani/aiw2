...
updateGraph :: Graph Exp -> Set Pattern -> Set Concept -> LTM -> Set Rewarding
                ...
                -> IO (Graph Exp,Set Pattern)
updateGraph graph' pats' _ _ _ _ _ _ _ dom exps | Map.null exps = return (graph',pats')
updateGraph graph' pats' concepts ltm' rew width depth redun consis dom exps = do
    --putStrLn "pats'"
    ...
    let exps' = filter (\x -> tsize x <= 7) . filter (\x -> (dom,x) `Set.notMember` pats') $ Map.keys exps
    let pats  = Set.union pats' $ Set.fromList $ zip (repeat dom) exps'
    let substitutions = map fst $ Map.elems exps
    units <- sample' (oneof $ map return [c | (C (0,_,_,_,_,d,Root c)) <- Set.toList concepts,d==dom])
    --putStrLn "exps'"
    ...
    let graph1
            = if null exps'                             -- if all exps already in graph
              then graph'                       -- then return same
              else  let newGraph = [((dom,x,y),(Satisfiable,Set.empty))
                                                | (d1,x) <- Set.toList pats,
                                                  (d2,y) <- Set.toList pats,
                                                  d1==dom,
                                                  d2==dom,
                                                  x /= y,
                                                  not (isVar x),
                    ...
                    in Map.unionWithKey (\(d,e1,e2) (a,s1) (b,s2) ->
                                                let vrs = getVars e1
                                                    vrs' = nub' vrs
...
computeByAnalogy
        ...
        item@(Item dom t t' v target)
  | t == t' = return "Lhs and Rhs are the same!"
  | containsVar t = return "Open question: no analagy found!"
  | not (containsVar t) = do
    let isOpen = isVar t'
    let concepts = Set.toList concepts'
    let domains = nub' [d | C (_,_,_,_,_,d,_) <- concepts,
                            if isNothing target then True else d == (fromJust target)]
    let tConcepts = nub' (getConcepts t ++ getConcepts t')
    --let count = length tConcepts
    let count = length $ nub' [(i,x) | C (i,_,_,_,_,d,x) <- concepts, d==dom]
    let translations = concat [generateTranslations dom d 3 tConcepts concepts
                                     | d <- domains]
                                       -- count <- [1..5]]
    let applyTranslation trans term =
            foldl (\term1 (i,a,b) -> replaceConcepts i a b term1) term trans
-- function validTranslation verifies that the translation makes the current item computable
    let validTranslation (d,[]) = False
    let validTranslation (d,tr) =
            let t1 = applyTranslation tr t
                t2 = applyTranslation tr t'
            in if isVar t'
                then isJust $ getAnswer width depth concepts' ltm rew' t1
                else let (answer,_) = solveAnswer width depth concepts' ltm rew' (t1,Nothing,Set.empty) (Just t2)
                     in (isJust answer) && (fromJust answer) == t2
-- get score for translation: number of preserved facts
    let facts = filter axIsFact . Set.toList $ ltmFacts ltm
    let getScore (_,[]) = 0
    let getScore (dm,tr) = -- getScore :: (Domain,Translation) -> (Axiom,Int)
          length
          . filter (==True)
          . map (\x -> Set.member x $ ltmFacts ltm)
          $ [ Axiom (w,x,True,y,z, ((applyTranslation tr lhs),(dm,dir),(applyTranslation tr rhs)) )
                    | Axiom (w,x,True,y,z,(lhs,(dom',dir),rhs)) <- facts,
                      dom' == dom]
    let getProof trans = 
            let t1 = applyTranslation trans t
                t2 = applyTranslation trans t'
            in if isVar t'
                then let ans = solve width depth concepts' ltm rew' (t1,Nothing,Set.empty) Nothing
                     in if isJust ans && notnull (fromJust ans)
                         then (t1,Nothing,Set.empty) : fromJust ans
                         else []
                else findSolution concepts' ltm rew' width depth (t1,t2)
-- valid translations: that make the current item computable
    let valid = filter validTranslation translations
-- select the best translation: shorter and with higher score (preservation of existing facts)
    let compareT (i,(_,x)) (j,(_,y)) = if i==j then compare (length x) (length y) else compare j i
    let preserveScore = sortBy compareT [(getScore v,v) | v <- valid]
    if null preserveScore
    then return "No analogy found."
    else do
      let maxScore = fst $ head preserveScore
      let (preserved,(domain,translation)) = head $ trace (unlines $ map show $ takeWhile (\(x,_) -> x == maxScore) preserveScore) preserveScore
      let result = unlines . map (\(_,x,y) -> x ++ " -> " ++ y) $ translation
      let proof = let p = getProof translation
                  in if null p
                      then ""
                      else "\n" ++ showSolution p
                      
      let result' = "True with following analogy in target domain " ++ domain
                      ++ ":\n"
                      ++ result ++ "\n"
                      ++ show preserved ++ " facts preserved.\n"
                      ++ proof
      return result'
  | otherwise = do
...
generateTranslations source target count cs concepts = 
  let singles = [ (i,c1,c2)
                    | -- (i,c1) <- cs,
                      C (i,_,_,_,_,d1,Root c1) <- concepts,
                      C (j,_,_,_,_,d2,Root c2) <- concepts,
                      i==j,
                      c1/=c2,
                      d1==source,
                      d2==target]
...
findDelta :: Bool -> Verbose -> UTCTime -> Agent -> Item -> 
             IO (Graph Exp,Set Pattern,LTM,Bool,Set Rewarding,[(Int,Axiom)],(RecentlyRemoved,[StateD],[(String,Exp)]))
findDelta test vb time
          agent@(Agent c (width,depth,ltmSize,(redun,consistency,mode,iter)) (ltm,concepts,graph',pats',rew',rem')) 
          item@(Item dom t t' v _)
-- input and ouput are same, so no need to perform computation
  | t == t' = do 
    ...
    let ltm' = insertInLTM ltm $ Axiom (fromIntegral v,1,True,time,time,(t,(dom,Bi),t'))
    (graph,pats) <- updateGraph graph' pats' concepts ltm' rew width depth redun consistency dom newShapes
...
-- variable in the question: abduction
  | (containsVar t' || containsVar t) && v >= 0 = do
    let axiom = Axiom (fromIntegral v,1,True,time,time,(t,(dom,Bi),t'))
...
-- negative item
  | v < 0 && isGoal' rew' t' = do
    ...
    let positiveUpdate = if null badAxioms
                         then Set.empty
                         else Set.singleton $ Axiom (fromIntegral v,1,True,time,time,(t,(dom,Neg),t'))
...
-- small fact with unary function
  | isUnary' t && size t < 3
    && ltmNotmember (Axiom (fromIntegral v,1,True,time,time,(t,(dom,Bi),t'))) ltm = do
    let positiveAxiom = Axiom (fromIntegral v,1,True,time,time,(t,(dom,Bi),t'))
    let rew = updateRewarding rew' item
    -- let newShapes = Map.filterWithKey (\k a -> null $ getUnary k) $ makeShapes (t,t')
    -- (graph,pats) <- updateGraph graph' pats' concepts ltm rew width depth redun dom newShapes
  ...
  | otherwise = {-# SCC "findDelta" #-} do  -- positive item
    let positiveAxiom = if v >= 0 then Axiom (fromIntegral v,1,True,time,time,(t,(dom,Bi ),t'))
                                  else Axiom (fromIntegral v,1,True,time,time,(t,(dom,Neg),t'))
    let optimumScore   = v
    let concepts' = Set.size $ Set.filter isUnitConcept concepts
    ...
    (graph,pats) <- updateGraph graph' pats' concepts (insertInLTM ltm positiveAxiom) rew width depth redun consistency dom newShapes -- foldl insertInShapes graph' $ map fst newShapes
...
-- Linguistic reasoning: find synonyms
        ...
        let isSynonymous i a b =
                if (Axiom (0,0,True,time,time,(Root a,(dom,Bi),Root b))) `elem` theoryFacts
                then True
                else let s =  [f | f <- filter (\x -> axIsFact x && not (axIsNeg x)) theoryFacts,
                                   let concepts = getConcepts $ axLhs f,
                                   (i,a) `elem` concepts,
                                   --size (axLhs f) > 1,
                                   --snd' (axiom' f) /= Neg,
                                   let r = filter isJust [runContext (axLhs f) c a b | (j,c) <- (nub' . getConcepts $ axLhs f),i==j],
                                   notnull r,
                                   and (map fromJust r)]
                  in notnull s && notnull (tail s) -- && length s >= 3
        let synonyms = if (Rew (0,"OK")) `Set.member` rew
                       then [Axiom (fromIntegral v,1,False,time,time,(Root c,(dom,Bi),Root syn))
                                   | (i,c) <- nub' $ getConcepts t,
                                     syn <- [c' | C (j,_,_,_,_,d,Root c') <- Set.toList concepts,d==dom,i==j, c /= c', isSynonymous i c' c]
                            ]
                       else []
-- Analogical reasoning: find translations
        let checkContext exp source a b = 
                let x = replaceAllSubExp2 exp (Root a) (Root source)
                    y = replaceAllSubExp2 exp (Root b) (Root source)
                    ltm' = insertInLTM ltm positiveAxiom
                    (a',_) = solveAnswer width 1 concepts ltm' rew (x,Nothing,Set.empty) (Just $ Root "T")
                    (b',_) = solveAnswer width 1 concepts ltm' rew (y,Nothing,Set.empty) (Just $ Root "T")
                in
                    if isNothing a' || isNothing b'
                        then Nothing
                        else Just (a' == b')

        let checkTranslation i a b =
                if (Axiom (0,0,True,time,time,(Root a,(dom,Bi),Root b))) `elem` theoryFacts
                then True
                else let x = replaceAllSubExp2 t (Root b) (Root a)
                         x' = solveAnswer width 3 concepts ltm rew (x,Nothing,Set.empty) (Just $ Root "T")
                     in isJust (fst x') 

        let translations = if (Rew (0,"T")) `Set.member` rew && v >= 1
                           then [Axiom (fromIntegral v,1,False,time,time,(Root c,(dom,Bi),Root syn))
                                       | (i,c) <- nub' $ getConcepts t,
                                         syn <- [c' | C (j,_,_,_,_,d,Root c') <- Set.toList concepts,d==dom,i==j, c /= c', checkTranslation i c' c]
                                ]
                           else []

        let mixed = [ax | ax@(Axiom (_,_,_,_,_,(lhs,_,rhs))) <- generateMixedAxioms time concepts dom,
                          null [0 | f <- theoryFacts, axIsNeg f, f `isInstanceOf` ax],
                          satisfy concepts'
                                  [map snd (fromJust r)
                                        | f <- theoryFacts,
                                          axIsBi f,
        ...
        let mixedPlus = [ax |   ax@(Axiom (_,_,_,_,_,(lhs,_,rhs))) <- generateFuncsAbs time [positiveAxiom] dom,
        ...
        let algebraic = [Axiom (0,1,False,time,time,(x,(dom,Bi),y))
                            |   ((d,x,y),(Valid,set)) <- Map.toList graph,
                                d==dom,
                                -- (not . Set.null $ set),
                                ltmNotmember (Axiom (0,1,False,time,time,(x,(dom,Bi ),y))) ltm,
                                ltmNotmember (Axiom (0,1,False,time,time,(x,(dom,Uni),y))) ltm
        ...
        let prevRecExamples = if isUnary'' . fst' . axiom' $ positiveAxiom
                              then [(((Unary (Oper f) x),right),ltmDelete ax ltm)
                                        | ax@(Axiom (_,_,_,_,_,((Unary (Oper f) x),(dom,Bi),right))) <- Set.toList $ ltmFacts ltm,
                                          f == getUnaryOper t]
                              else []
        ...
        let deltas = -- remove redundant candidates
                     ...
                     $ nub' (facts ++ mixed ++ mixedPlus ++ algebraic ++ synonyms) -- ++ translations) -- ++ recursive)
...
findConvergence vb width depth concepts ltm rew ax@(Axiom (_,_,_,_,_,(x,(dom,dir),y))) =
    let ltmClosed = Ltm (ltmFacts ltm) Set.empty
        ...
        consts = [e | C (0,_,_,_,_,d',(Root e)) <- Set.toList concepts,d'==dom]
...
hasNoDivergence vb width depth concepts ltm' rew ax@(Axiom (_,_,_,_,_,(x,(dom,dir),y))) = do
   ...
   then return True
    ...
    let vars = nub' vars'
    let units = [e | C (0,_,_,_,_,d',e@(Root _)) <- Set.toList concepts,d'==dom]
    let substitutions' = 
...
theoryUpdate vb width depth ltmSize concepts rew graph ltm rem delta = do
    time <- getCurrentTime
    let invalid = Set.fromList [Axiom (0,1,False,time,time,(x,(dom,Bi),y))
                                        |   ((dom,x,y),(Invalid,_)) <- Map.toList graph,
                                            ltmMember (Axiom (0,1,False,time,time,(x,(dom,Bi),y))) ltm]
    ...
    let neg3 = Set.filter (\(Axiom (_,_,_,_,_,(x,(dom,_),y))) -> 
                                            if (dom,x,y) `Map.member` graph 
                                            then (\z -> fst z == Invalid) (graph Map.! (dom,x,y)) 
...
generateMixedAxioms time concepts' dom =
    let v = makeV "x"
        ...
        consts = [e | C (0,_,_,_,_,d,e@(Root _)) <- concepts,d==dom]
        unary  = [e | C (1,_,_,_,_,d,e@(Root _)) <- concepts,d==dom]
        binary = [e | C (2,_,_,_,_,d,e@(Root _)) <- concepts,d==dom]
        exps1 = [Axiom (0,1,False,time,time,(exp,(dom,Bi),c)) | i <- [2..4], c <- consts, exp <- generateExps i [v,c] unary binary]
...
generateAlgebraic time concepts = 
    let vars = [makeV [v] | v <- "xyz"]
        unary  = nub' [e | C (arit,_,_,_,_,_,e@(Root f)) <- concepts, arit==1]
        binary = nub' [e | C (arit,_,_,_,_,_,e@(Root f)) <- concepts, arit==2]
...
generateFuncsRec ltm width depth special time concepts' base@(Axiom (_,_,_,_,_,(t@(Unary f c),(dom,dir),t'))) = 
    let concepts = Set.toList concepts'
        d = 6
        vars = [makeV [c] | c <- "xyz"]
        units = nub' $ vars ++ [exp | C (arit,_,_,_,_,d',exp) <- concepts,d'==dom,arit==0, size exp == 1]
        binary = [exp | C (arit,_,_,_,_,d',exp) <- concepts,d'==dom,arit==2]
        ...
        small =     filter (\x -> not . fst $ findAnswerDecl Set.empty ltm Set.empty width depth (dom,x,lhs))
        ...
        result = filter (/= base)
                 . nub'
                 . filter isPure
                 $ [Axiom (0,1,False,time,time,(lhs,(dom,Bi),rhs))
...
generateFuncsAbs _ [] _ = []
generateFuncsAbs time axioms dom = 
    let vars = [makeV [c] | c <- "xyz"]
        exps = nub' $
               [Axiom (0,1,False,time,time,((foldl (\x' (var',sub') -> replaceAllSubExp2 x' var' sub') x v),(dom,dir),(foldl (\x' (v',s') -> replaceAllSubExp2 x' v' s') y v))) -- replace sub with v in ip
                  | (Axiom (_,_,_,_,_,(x,(d,dir),y))) <- axioms,
                    d == dom,
...
generateFacts' time 3 dom concepts = 
    let consts = nub' [exp | C (arit,_,freq,_,_,d',exp) <- concepts, arit==0,dom==d']
        unary  = nub' [exp | C (arit,_,freq,_,_,d',Root exp) <- concepts, arit==1,dom==d']
        facts  = nub' [Axiom (0,1,False,time,time,((makeU u c),(dom,Bi),d)) | c <- consts, u <- unary, d <- consts]
    in filter isFact facts
generateFacts' time 4 d'' concepts = 
    let consts = nub' [e | C (arit,_,freq,_,_,d',e) <- concepts, arit==0,d''==d']
        unary  = nub' [e | C (arit,_,freq,_,_,d',Root e) <- concepts, arit==1,d''==d']
        binary = nub' [e | C (arit,_,freq,_,_,d',Root e) <- concepts, arit==2,d''==d']
        factsB = nub' [Axiom (0,1,False,time,time,((makeB b c d),(d'',Bi),e))
                        | c <- consts,  d <- consts, b <- binary, e <- consts]
        factsU = nub' [Axiom (0,1,False,time,time,((makeU u1 c),(d'',Bi),(makeU u2 d)))
                        | c <- consts, u1 <- unary, d <- consts, u2 <- unary]
    in filter isFact $ factsB ++ factsU
generateFacts' time 5 d'' concepts = 
    let consts = nub' [e | C (arit,_,freq,_,_,d',e) <- concepts, arit==0,d''==d']
        unary  = nub' [e | C (arit,_,freq,_,_,d',Root e) <- concepts, arit==1,d''==d']
        binary = nub' [e | C (arit,_,freq,_,_,d',Root e) <- concepts, arit==2,d''==d']
        facts = nub' [Axiom (0,1,False,time,time,((makeB b c d),(d'',Bi),(makeU u e)))
                        | b <- binary, u <- unary,
                          c <- consts,  d <- consts, e <- consts]
    in filter isFact facts
generateFacts' time 6 d'' concepts = 
    let consts = nub' [e | C (arit,_,freq,_,_,d',e) <- concepts, arit==0,d''==d']
        binary = nub' [e | C (arit,_,freq,_,_,d',Root e) <- concepts, arit==2,d''==d']
        facts = nub' [Axiom (0,1,False,time,time,((makeB b1 c d),(d'',Bi),(makeB b2 e f)))
                        | b1 <- binary, b2 <- binary,
                          c <- consts,  d <- consts,
                          e <- consts,  f <- consts]
    in filter isFact facts
...
generateRelevant time concepts = 
    let vars   = [makeV [v] | v <- "zyx"]
        domains = nub' [d | C (_,_,_,_,_,d,_) <- concepts]
        relevant d = 
            let consts = nub' [makeR e | C (arit,_,freq,_,_,d',Root e) <- concepts, arit==0,d'==d]
                binary = nub' [e | C (arit,_,freq,_,_,d',Root e) <- concepts, arit==2,d'==d]
            in  nub' $ concat
                        [[Axiom (0,1,False,time,time,((makeB b v1 v2),(d,Bi),c)),
                          Axiom (0,1,False,time,time,((makeB b v1 c),(d,Bi),v2)),
                          Axiom (0,1,False,time,time,((makeB b c v2),(d,Bi),v1))]
                            | c <- consts, b <- binary,
                              v1 <- vars, v2 <- vars]
...
generateFuncsAll time concepts len pos =
    let variables = "zyx"
        parts = [lhs | (Item _ lhs _ _ _) <- pos] ++ [rhs | (Item _ _ rhs _ _) <- pos]
        units = nub' $ [x | x <- (concatMap getSubExp parts), size x == 1]
                      ++ [makeV [c] | c <- variables]
                      ++ [exp | C (arit,_,freq,_,_,_,exp) <- concepts, arit==0]
                                -- size exp == 1]
        unary  = nub' [exp | C (arit,_,freq,_,_,_,exp) <- concepts, arit==1] -- ,size exp == 1]
                    -- ++ [HsVar x | (HsApp (HsVar x) _) <- parts]
        binary = nub' [exp | C (arit,_,freq,_,_,_,exp) <- concepts, arit==2] -- ,size exp == 1]
