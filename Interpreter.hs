...
findAnswerDecl concepts ltm rew width depth (dom,exp,rhs) =
...
applyFact b wm a@(Axiom (_,_,_,_,_,(lhs,(_,Uni),rhs))) -- surface application of shallow rules
  | wm == lhs && b = [(rhs,Just a)]
  | otherwise = []
...
applyRule b func a@(Axiom (_,_,_,_,_,(lhs,(_,Uni),rhs)))
